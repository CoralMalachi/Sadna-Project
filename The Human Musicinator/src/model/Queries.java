...
public final class Queries {
     ...
     */
     ...
     static {
        ...
        // RANDOM_ID_QUERY_MAP.put("idOnlyGroupQuery","select artist.id,artist.name from artist where artist.type=(select id from artist_type where artist_type.name=\"Group\") order by rand() limit 1");
         RANDOM_ID_QUERY_MAP.put("idArtistQuery","select artist.id,artist.name from artist inner join artist_meta on artist_meta.id=artist.id where artist_meta.rating>85 and artist_meta.rating_count>3 order by rand() limit 1");
