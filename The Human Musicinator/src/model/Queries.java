...
public final class Queries {
     ...
     */
     ...
     public static final Map<String, String> QUERY_RANDOM_MAP;
     static {
         QUERY_RANDOM_MAP= new HashMap<String, String>();
         QUERY_RANDOM_MAP.put("idOnlyArtistQuery","select artist.id,artist.name from artist inner join artist_meta on artist_meta.id=artist.id where artist_meta.rating>78 and artist_meta.rating_count>3 and artist.type=(select id from artist_type where artist_type.name=\"Person\") order by rand() limit 1");
         QUERY_RANDOM_MAP.put("idOnlyGroupQuery","select artist.id,artist.name from artist inner join artist_meta on artist_meta.id=artist.id where artist_meta.rating>78 and artist_meta.rating_count>3 and artist.type=(select id from artist_type where artist_type.name=\"Group\") order by rand() limit 1");
     }
     static {
...
//             QUERY_MAP.put("idOnlyGroupQuery", "SELECT artist.id,artist.name FROM artist where artist.type=(select id from artist_type where artist_type.name=\"Group\") ORDER BY RAND() LIMIT 1");
             ...
             QUERY_MAP.put("englishAlbumOfArtist","select `release`.id,`release`.name from `release` INNER JOIN release_group ON release_group.id=`release`.release_group and release_group.type=(select id from release_group_primary_type where release_group_primary_type.name=\"Album\")\n" +
             ...
             QUERY_MAP.put("englishSingleOfArtist","select `release`.id,`release`.name from `release` INNER JOIN release_group ON release_group.id=`release`.release_group and release_group.type=(select id from release_group_primary_type where release_group_primary_type.name=\"Single\")\n" +
                 "              where `release`.language=(select id from language where language.name=\"English\") and `release`.artist_credit= ? ORDER BY RAND() LIMIT 1");
             ...
             QUERY_MAP.put("numberOfArtistAlbumsQuery","select DISTINCT count(`release`.id) from `release` INNER JOIN release_group ON release_group.id=`release`.release_group and release_group.type=(select id from release_group_primary_type where release_group_primary_type.name=\"Album\") \n" +
                    "             and `release`.artist_credit= ?");

             QUERY_MAP.put("numberOfSingleAlbumsQuery","select DISTINCT count(`release`.id) from `release` INNER JOIN release_group ON release_group.id=`release`.release_group and release_group.type=(select id from release_group_primary_type where release_group_primary_type.name=\"Single\") \n" +
                 "             and `release`.artist_credit= ?");

             QUERY_MAP.put("mostFreqReleasesAreaOfArtistQuery","select area.name from area where area.id=(\n" +
                     "-- \n" +
             ...
             QUERY_MAP.put("mostActiveYearArtistQuery","select release_country.date_year from release_country \n" +
                     "inner join `release` on `release`.id=release_country.release\n" +
                     "where `release`.artist_credit= ? group by date_year order by count(`release`.name) DESC limit 1");

             QUERY_MAP.put("mostFreqLanguageAlbumsArtist","select language.name from language where language.id=(\n" +
                     "\n" +
             ...
             QUERY_MAP.put("mostFreqLanguageSinglesArtist","select language.name from language where language.id=(\n" +
                 "\n" +
                 "select language from `release` \n" +
                 "INNER JOIN release_group ON release_group.id=`release`.release_group\n" +
...
//             QUERY_MAP.put("albumRandomQuery", "select `release`.name from `release` INNER JOIN release_group ON release_group.id=`release`.release_group and release_group.type=(select id from release_group_primary_type where release_group_primary_type.name=\"Album\") ORDER BY RAND() LIMIT 1");
         ...
         //fixed!
              QUERY_MAP.put("artistAliasNameQuery", "select artist_alias.name from artist_alias where artist_alias.artist= ? order by artist_alias.last_updated DESC limit 1");
